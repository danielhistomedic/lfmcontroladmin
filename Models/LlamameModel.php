 ...
 * Clase LlamameModel
 */
class LlamameModel extends Mysql
{
    ...
    /**
     * Método Constructor de LlamameModel.
    ...
    public function selectRecords(): array
    {
        ...
        try {
            ...
            [ Instruccion sql ]*/
            $sql = "SELECT ";
            $sql .= "t.*, us.usuario, s.name as servicio, CONCAT_WS(' ', dat_gen_uat.nombre, dat_gen_uat.paterno, dat_gen_uat.materno) as usuario_atendio ";
            $sql .= "FROM " . LlamameModel::CONSTANTES_MODEL['tabla'] . " t ";
            $sql .= "INNER JOIN usuarios us ON (us.id = t.usuario_id_updated) ";
            $sql .= "INNER JOIN servicios s ON (s.id = t.servicio_id) ";
            $sql .= "LEFT JOIN usuarios_datos_generales dat_gen_uat ON (dat_gen_uat.usuario_id = t.usuario_id_atendio) ";
    ...
    public function selectRecord(int $record_id): array
    {
        ...
        try {
            ...
            [ Instruccion sql ]*/
            $sql = "SELECT ";
            $sql .= "t.*, CONCAT_WS(' ', dat_gen.nombre, dat_gen.paterno, dat_gen.materno) as usuario, s.name as servicio, CONCAT_WS(' ', dat_gen_uat.nombre, dat_gen_uat.paterno, dat_gen_uat.materno) as usuario_atendio ";
            $sql .= "FROM " . LlamameModel::CONSTANTES_MODEL['tabla'] . " t ";
    ...
    public function validInsertExist(string $texto_valida): bool
    {
        ...
        try {
            ...
            /*-------------------------------------------
            [ Instruccion sql ]*/
            $sql = "SELECT id FROM " . LlamameModel::CONSTANTES_MODEL['tabla'] . " ";
            $sql .= "WHERE ";
    ...
    public function validUpdateExistRecord(string $texto_valida, int $id): bool
    {
        ...
        try {
            ...
            [ Instruccion sql ]*/
            $sql = "";
            $sql .= "SELECT * FROM " . LlamameModel::CONSTANTES_MODEL['tabla'] . " ";
            $sql .= "WHERE ";
     ...
     * 
     * @param model &$model
     * Envío del modelo \LlamameModel por referencia, para asiganr el valor lastInsertId al modelo.
     *
     * @return bool $response
     ...
     * 
     */
    public function insertRecord(LlamameModel &$modelo): bool
    {
     ...
     * 
     * @param object LlamameModel $model
     * Envío del modelo por referencia con los datos requeridos para el insert
     *
     */
    public function recordCreate(LlamameModel &$modelo): int
    {
        ...
        /*-------------------------------------------
            [ Instruccion sql ]*/
        $sql = "INSERT INTO " . LlamameModel::CONSTANTES_MODEL['tabla'] . " SET ";
     ...
     * 
     */
    private function insertAdjunto(LlamameModel $modelo): bool
    {
        ...
        try {
            ...
            $ruta_doctos = "Assets/files/llamame/";
     ...
     * 
     */
    public function updateRecord(LlamameModel $modelo): bool
    {
     ...
     *
     */
    public function recordUpdate(LlamameModel &$modelo): bool
    {
        ...
        /*-------------------------------------------
            [ Instruccion sql ]*/
        $sql = "UPDATE " . LlamameModel::CONSTANTES_MODEL['tabla'] . " SET ";
        $sql .= "empresa_id = :empresa_id, ";
        $sql .= "sucursal_id = :sucursal_id, ";
        $sql .= "name = :name, ";
        $sql .= "telefono = :telefono, ";
     ...
     * 
     */
    public function updateEstatusRecord(LlamameModel $model): bool
    {
    ...
    /**
     * Marca como atendida un Registro determinado.
     * 
     * @param object $modelo
     * Envío del modelo por valor, que contine la información a actualizar y 
     * los parámetros condicionales para realizar el update.
     ...
     * 
     */
    public function marcarAtendidaSolicitudLlamada(LlamameModel $model): bool
    {
        ...
        try {
            ...
            $sql .= "usuario_id_atendio = :usuario_id_atendio ";
            $sql .= "WHERE  ";
            $sql .= "id = :id ";
            ...
            $arrData = [
                'id' => $model->getId(),
                'atendida' => $model->getAtendida(),
                'usuario_id_atendio' => $model->getUsuarioIdAtendio()
            ];
