...
class Consumer
{
    ...
    public function request(string $expression): array
    {
        $responseConsumer = $this->getClient()->consume($this->getUri(), $expression);

        $arrayQuery = array(
            'CodigoEstatus' => $responseConsumer->get('CodigoEstatus'),
            'Estado' =>   $responseConsumer->get('Estado'),
            'EsCancelable' =>   $responseConsumer->get('EsCancelable'),
            'EstatusCancelacion' =>   $responseConsumer->get('EstatusCancelacion'),
            'ValidacionEFOS' =>   $responseConsumer->get('ValidacionEFOS')
        );
