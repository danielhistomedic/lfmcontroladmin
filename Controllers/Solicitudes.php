...
class Solicitudes extends Controllers
{
    ...
    const prefijo_msj_error = "solicitudes_clientes";

    /**
     * Método Constructor de Controlador Solicitudes.
    ...
    public function Solicitudes()
    {
        ...
        try {
            ...
            $this->permisosMod = $arrPermisos[MOD_SOLICITUDES];

            /*-------------------------------------------
            [ Obtener datos de Modulo ]*/
            $menus_model = new MenusModel;
            $menu = $menus_model->selectMenu(MOD_SOLICITUDES);
    ...
    public function getListaRecords()
    {
        ...
        try {
            ...
            $class_model = new SolicitudesModel;
            ...
            for ($i = 0; $i < count($arrData); $i++) {
                ...
                if ($atendida == 0) {
                    $arrData[$i]['atendida'] = 'SIN ATENDER';
                } else {
    ...
    public function setRecord()
    {
        ...
        try {
            ...
            $atendida = strclean($_POST['atendida']);
            $comentarios = strclean($_POST['comentarios']);

            /*-------------------------------------------
            [ Valida formulario ]*/
            ($record_id == 0) ? die(json_encode(getResponse("Debe seleccionar un registro"), JSON_UNESCAPED_UNICODE)) : "";
            (trim($atendida) == '') ? die(json_encode(getResponse("Debe indicar si fue atendido"), JSON_UNESCAPED_UNICODE)) : "";
            (trim($comentarios) == '') ? die(json_encode(getResponse("Debe indicar comentarios"), JSON_UNESCAPED_UNICODE)) : "";
            ...
            $class_model->setComentarios($comentarios);
