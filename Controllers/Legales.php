 ...
 * Controlador Legales 
 */
class Legales extends Controllers
{

    private $session;
    private $permisosMod;

    const prefijo_msj_error = "legales";

    /**
     * Método Constructor de Controlador Legales.
    ...
    public function Legales()
    {
        ...
        try {
            ...
            $this->permisosMod = $arrPermisos[MOD_LEGALES];

            /*-------------------------------------------
            [ Obtener datos de Modulo ]*/
            $menus_model = new MenusModel;
            $menu = $menus_model->selectMenu(MOD_LEGALES);
    ...
    public function setRecord()
    {
        ...
        try {
            ...
            $terminos = strclean($_POST['terminos']);
            $aviso = strclean($_POST['aviso']);
            $envio = strclean($_POST['envio']);

            /*-------------------------------------------
            [ Valida formulario ]*/
            (trim($terminos) == '') ? die(json_encode(getResponse("Debe indicar terminos y condiciones"), JSON_UNESCAPED_UNICODE)) : "";
            (trim($aviso) == '') ? die(json_encode(getResponse("Debe indicar aviso de privacidad"), JSON_UNESCAPED_UNICODE)) : "";
            (trim($envio) == '') ? die(json_encode(getResponse("Debe indicar politicas de envio"), JSON_UNESCAPED_UNICODE)) : "";
            ...
            $class_model->setEmpresaId($empresa_id);
            $class_model->setSucursalId($sucursal_id);
            $class_model->setTerminos($terminos);
            $class_model->setAviso($aviso);
            $class_model->setEnvio($envio);
