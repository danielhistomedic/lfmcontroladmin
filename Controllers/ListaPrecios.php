...
/**
 * Controlador ListaPrecios 
 */
class ListaPrecios extends Controllers
{
    ...
    /**
     * Método Constructor de Controlador ListaPrecios.
    ...
    public function ListaPrecios()
    {
        ...
        try {
            ...
            $this->permisosMod = $arrPermisos[MOD_CAT_LISTA_PRECIOS];

            /*-------------------------------------------
            [ Obtener datos de Modulo ]*/
            $menus_model = new MenusModel;
            $menu = $menus_model->selectMenu(MOD_CAT_LISTA_PRECIOS);
            ...
            $data['menu'] = MOD_CAT_LISTA_PRECIOS;
    ...
    public function getListaRecords()
    {
        ...
        try {
            ...
            $class_model = new ListaPreciosModel;
    ...
    public function getSelectRecords()
    {
        ...
        try {
            ...
            $lp_model = new ListaPreciosModel;
            $arrData = $lp_model->selectRecords();
            if (count($arrData) > 0) {
                for ($i = 0; $i < count($arrData); $i++) {
                    if ($arrData[$i]['activo'] == 1) {
                        $htmlOptions .= '<option value="' . $arrData[$i]['id'] . '">' . $arrData[$i]['name'] . ' - ' . $arrData[$i]['descripcion'] . '</option>';
                    }
                }
            }
        } catch (\Throwable $th) {
    ...
    /**
     * Obtiene la lista de registros, para generar la vista de lista de precios en form de productos.
     * 
     * @return string $htmlOptions
     * 
     */
    public function getInputsListaPreciosProductos()
    {
        ...
        try {
            ...
            if (count($arrData) > 0) {
                for ($i = 0; $i < count($arrData); $i++) {
                    if ($arrData[$i]['activo'] == 1) {
                        if ($i == 0) {
                            $htmlOptions .= '<div class="form-group row align-items-center pb-3">';
                        } else {
                            $htmlOptions .= '<div class="form-group row align-items-center pt-3 pb-3">';
                        }
                        $htmlOptions .= '   <label class="col-lg-4 col-xl-3 control-label text-lg-end mb-0">' . $arrData[$i]['name'] . ':</label>';
                        $htmlOptions .= '   <div class="col-lg-3">';
                        $htmlOptions .= '       <input type="hidden" id="lista_precios_id_' . $i . '" name="lista_precios_id[]" value="' . $arrData[$i]['id'] . '">';
                        $htmlOptions .= '       <input type="text" data-id="' . $arrData[$i]['id'] . '" title="' . $arrData[$i]['descripcion'] . '" class="form-control precios" id="price_' . $i . '" name="precios[]" value="" placeholder="' . $arrData[$i]['descripcion'] . '" required />';
                        $htmlOptions .= '   </div>';
                        $htmlOptions .= '</div>';
                    }
                }
    ...
    /**
     * Obtiene la lista de registros, para generar la vista de lista de precios en vista de productos.
     * 
     * @return string $htmlOptions
     * 
     */
    public function getListaPreciosProductosVista()
    {
        ...
        try {
            ...
            if (count($arrData) > 0) {
                for ($i = 0; $i < count($arrData); $i++) {
                    if ($arrData[$i]['activo'] == 1) {
                        if ($i == 0) {
                            $htmlOptions .= '<div class="form-group col-12 col-sm-6 col-lg-4 col-xxl-3 pt-3">';
                        } else {
                            $htmlOptions .= '<div class="form-group col-12 col-sm-6 col-lg-4 col-xxl-3">';
                        }

                        $htmlOptions .= '   <label class="control-label ">Precio ' . $arrData[$i]['name'] . ':</label>';
                        $htmlOptions .= '   <p class="fst-italic text-3 m-0">' . $arrData[$i]['descripcion'] . ':</p>';
                        $htmlOptions .= '   <p class="mt-0 text-secondary precios_read" data-id="' . $arrData[$i]['id'] . '" id="estatus_read_' . $i . '"></p>';
                        $htmlOptions .= '</div>';
                    }
                }
            }
