...
class Suscripciones extends Controllers
{
    ...
    /**
     * Método Constructor de Controlador Suscripciones.
    ...
    public function Suscripciones()
    {
        ...
        try {
            ...
            $this->permisosMod = $arrPermisos[MOD_SUSCRIPCIONES];

            /*-------------------------------------------
            [ Obtener datos de Modulo ]*/
            $menus_model = new MenusModel;
            $menu = $menus_model->selectMenu(MOD_SUSCRIPCIONES);
    ...
    public function getListaRecords()
    {
        ...
        try {
            ...
            $class_model = new SuscripcionesModel;
    ...
    public function getRecord()
    {
        ...
        try {
            ...
            getLoggerSystem()->error(getMensajeError($th));
            die(json_encode(getResponse('Code suscripciones_1001. Error Desconocido'), JSON_UNESCAPED_UNICODE));
        }
    ...
    public function setRecord()
    {
        ...
        try {
            ...
            $slug = str_replace(" ", "_", $email_lowercase);
            ...
            if ($observaciones == '') {
                die(json_encode(getResponse("Debe indicar observaciones"), JSON_UNESCAPED_UNICODE));
            }
            ...
            $class_model->setObservaciones($observaciones);
            ...
            if ($record_id == 0) {
                ...
                /*-------------------------------------------
                [ Valida si el Registro ya existe ]*/
                $existe = $class_model->validInsertExist($email);
                if ($existe == true) {
                ...
                /*-------------------------------------------
                [ Valida si el registro ya existe ]*/
                $existe = $class_model->validUpdateExistRecord($email, $record_id);
                if ($existe == true) {
    ...
    public function getSelectRecords()
    {
        ...
        try {
            ...
            $suscripciones_model = new SuscripcionesModel;
            $arrData = $suscripciones_model->selectRecords();
            if (count($arrData) > 0) {
                for ($i = 0; $i < count($arrData); $i++) {
