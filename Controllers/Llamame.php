 ...
 * Controlador Llamame 
 */
class Llamame extends Controllers
{
    ...
    /**
     * Método Constructor de Controlador Llamame.
    ...
    public function Llamame()
    {
        ...
        try {
            ...
            $this->permisosMod = $arrPermisos[MOD_LLAMAME];

            /*-------------------------------------------
            [ Obtener datos de Modulo ]*/
            $menus_model = new MenusModel;
            $menu = $menus_model->selectMenu(MOD_LLAMAME);
    ...
    public function getListaRecords()
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($arrData); $i++) {
                ...
                // atendida
                $atendida = $arrData[$i]['atendida'];
                if ($atendida == 0) {
                    $arrData[$i]['atendida'] = 'NO';
                } else {
                    $arrData[$i]['atendida'] = 'SI';
                }
                ...
                $btnAtendida = '<button type="button" class="mb-1 mt-1 me-1 btn btn-xs btn-outline-success" data-animation="' . $data_animation . '" onclick="fntAtendida(this)" data-id="' . openssl_encrypt($arrData[$i]['id'], METHODENCRIPT, KEY) . '" title= "Marcar como Atendida"><i class="fa-sharp fa-regular fa-check-to-slot "></i> </button>';
                // <i class="fa-sharp fa-regular fa-check-to-slot"></i>

                $btnView = ($this->permisosMod['r']) ? $btnView : '';
                $btnEdit = ($this->permisosMod['u']) ? $btnEdit : '';
                $btnDelete = ($this->permisosMod['d']) ? $btnDelete : '';

                $arrData[$i]['options'] = '' . $btnView . ' ' . $btnEdit  . ' ' . $btnDelete . ' ' . $btnAtendida;
            }
        } catch (\Throwable $th) {
            getLoggerSystem()->error(getMensajeError($th));
        }
    ...
    public function setRecord()
    {
        ...
        try {
            ...
            $servicio_id = intval(strclean($_POST['servicio_id']));
            ...
            if ($servicio_id == 0) {
                die(json_encode(getResponse("Debe indicar Servicio"), JSON_UNESCAPED_UNICODE));
            }
            ...
            $class_model->setServicioId($servicio_id);
    ...
    /**
     * Actualizar Estatus de Atendida
     * 
     * @response $arrResponse, donde:
     * * respuesta (string). Valores 'ok'/'error'. 'ok' en caso de que la respuesta sea existosa repsonde '', caso contrario = 'error'.
     * * mostrar_mensaje (bool). Valores true/false en caso de que se desee mostrar modal con los resultados de la respuesta.
    ...
    public function setAtendida()
    {
        ...
        try {
            ...
            [ Se reciben Datos del POST, y se limpian los datos recibidos ]*/
            $record_id = strClean($_POST['record_id']);
            $atendida = intval(strClean($_POST['atendida']));
            ...
            $class_model = new LlamameModel;
            $class_model->setId($record_id);
            $class_model->setAtendida($atendida);
            $class_model->setUsuarioIdAtendio($usuario_id_register);

            /*-------------------------------------------
            [ Elimina el Registro seleccionado. ]*/
            $response = $class_model->marcarAtendidaSolicitudLlamada($class_model);

            /*-------------------------------------------
            [ Evalúa respuesta  ]*/
            if ($response == true) {
                $arrResponse = getResponse('Registro marcado como atendido exitosamente', 'ok', true);
            } else {
                die(json_encode(getResponse('Error al marcar el registro, intente nuevamente'), JSON_UNESCAPED_UNICODE));
            }
        } catch (\Throwable $th) {
            getLoggerSystem()->error(getMensajeError($th));
            die(json_encode(getResponse('Code rol_1001. Error desconocido'), JSON_UNESCAPED_UNICODE));
