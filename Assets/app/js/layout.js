...
// function fillSelectEntidades() {

//     if (document.querySelector('.comboEntidades')) {
//         /*-------------------------------------------
//         [ Ajax LLenar Select de catalogo de roles ]*/
//         getFunctionHTML('Entidades', 'getAllSelect', '', function(responseHTML) {

//             document.querySelector('.comboEntidades').innerHTML = responseHTML;
//             /*-------------------------------------------
//             [ Inicializa Select2 ]*/
//             $('.comboEntidades').select2({
//                 language: "es",
//                 placeholder: 'Seleccione una opcion'
//             });
//             //Asignar Valor Default después de Inicializar Seleclt2
//             $('.comboEntidades').val('');
//             $('.comboEntidades').trigger('change');

//             $('.comboEntidades').on('select2:select', function(e) {
//                 var data = e.params.data;
//                 fillSelectMunicipios(data.id);
//             });
...
// function fillSelectPortabilidad() {

//     if (document.querySelector('#comboConbertura')) {

//         /*-------------------------------------------
//             [ Inicializa Select2 ]*/
//         $('#comboConbertura').select2({
//             language: "es",
//             placeholder: 'Seleccione una opcion',
//             minimumResultsForSearch: Infinity
//         });
//         //Asignar Valor Default después de Inicializar Seleclt2
//         $('#comboConbertura').val('');
//         $('#comboConbertura').trigger('change');
//     }
...
function fntExtraer(btnElement) {
    ...
    /*-------------------------------------------
    [ Deshabilita elemento para prevenir doble registro ]*/
    addLoadingButtonOpcionesDataTable(btnElement, 'fa-download')

    /*-------------------------------------------
     [ Ajax ]*/
    var formData = new FormData();
    formData.append('id', idRegistro);
    postFunctionData(formData, controllers, 'extraerDatos', function(responseObj) {

        // postFunctionData(form, 'extraerDatos', funcion, function(responseObj) {
        ...
        // --- Restablecer Icon ---
        removeLoadingButtonOpcionesDataTable(btnElement, 'fa-download');
...
function fntExtraerComplementario(btnElement) {
    ...
    var formData = new FormData();
    formData.append('id', idRegistro);
    postFunctionData(formData, controllers, 'extraerDatosComplementario', function(responseObj) {
...
function cargarDatosEdit(responseObj) {
    ...
    document.getElementById("titulo").value = responseObj.data.titulo;

    if (responseObj.data.adjunto_output != '') {
        document.getElementById('archivo_cargado_output').classList.remove('d-none');
    }
    // if (responseObj.data.adjunto_general != '') {
    //     document.getElementById('archivo_cargado_general').classList.remove('d-none');
    // }
    ...
    /*-------------------------------------------
    [ Validaciones ]*/
    if (responseObj.data.procesado == 1) {
        console.log(document.getElementById('btnGuardar'));
        var hasClase = document.getElementById('btnGuardar').classList.contains('disabled');
...
function cargarDatos(responseObj) {
    ...
    document.getElementById("id_record").value = responseObj.data.id;
    document.getElementById("titulo_read").innerHTML = responseObj.data.titulo;
    document.getElementById("sucursal_read").innerHTML = responseObj.data.sucursal;

    document.getElementById("adjunto_output_read").innerHTML = '<span><a style="text-decoration: underline;" href="' + assets + '/files/' + responseObj.data.adjunto_output + '" target="_blank">' + responseObj.data.adjunto_output + '</a></span>';
    // document.getElementById("adjunto_general_read").innerHTML = '<span><a style="text-decoration: underline;" href="' + assets + '/files/' + responseObj.data.adjunto_general + '" target="_blank">' + responseObj.data.adjunto_general + '</a></span>';
    ...
    if (responseObj.data.procesado == 1) {
        ...
        var hasClase = document.getElementById('btnEditar').classList.contains('disabled');
        console.log(hasClase);
        if (hasClase == false) {
