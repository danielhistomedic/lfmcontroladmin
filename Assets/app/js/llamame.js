...
function fillSelectServicios() {
    ...
    if (document.querySelector('#select_servicio_id')) {
        /*-------------------------------------------
        [ Ajax LLenar Select de catalogo ]*/
        getFunctionHTML('Servicios', 'getSelectRecords', '', function(responseHTML) {

            document.querySelector('#select_servicio_id').innerHTML = responseHTML;

            //Asignar Valor Default después de Inicializar Seleclt2
            $total_registros = document.getElementById('select_servicio_id').options.length;
            if ($total_registros > 1) {
                $('#select_servicio_id').val('');
                $('#select_servicio_id').trigger('change');
            } else {
...
function cargarDatosEdit(responseObj) {
    ...
    $('#select_servicio_id').val(responseObj.data.servicio_id);
    $('#select_servicio_id').trigger('change');

    /*-------------------------------------------
    [ Asigna imagen dropify  ]*/
    // document.getElementById('dropify_id').innerHTML = "";
    // var imagen = "Assets/files/llamame/" + responseObj.data.image;
...
function cargarDatos(responseObj) {
    ...
    document.getElementById("servicio_read").innerHTML = responseObj.data.servicio;
    document.getElementById("usuario_atendio_read").innerHTML = responseObj.data.usuario_atendio;
    document.getElementById("updated_atendio_read").innerHTML = responseObj.data.updated_atendio;
...
function setConfigTable(controlador, metodo) {
    ...
    configTable = {
        ...
        columnDefs: [
            { className: "text-center", targets: [0, 1, 2, 6, 8] },
